...
for modei = {'main'} 
  ...
  for nb = [30, 50];
   ...
   % end
    ...
    cap.xlabel = ['Recall'];
    cap.ylabel = ['Precision'];
    fig = make_err_plot({recs_mlh,       }, ... % recs_sh,           }, ...
		        {mean(precs_mlh,1),   }, ... % mean(precs_sh),    }, ...
		        {std(precs_mlh,0,1),}, ... % std(precs_sh,0,1), }, ...
		        {'MLH',                 }, ... % 'SH',              }, ...	
			cap, 'tr', 1);
...
for modei = {'main'}
 ...
 % pbre_mean = squeeze(mean(pbre,1));
  ...
  p = [pmlh_mean(:,R+1) ]; % psh_mean(:,R+1)];
  e = [pmlh_std(:,R+1) ]; % psh_std(:,R+1) ];
  n_lines = size(p,2);
  ...
  r =  [rmlh_mean(:,R+1) ]; % rsh_mean(:,R+1)];
  er = [rmlh_std(:,R+1)  ]; % rsh_std(:,R+1) ];
  n_lines = size(r,2);
